...
いきなり`LR(1)`を説明するのは難しいので、まずはもっと簡略化した`SLR(1)`について説明します。その前に、4.3で概略を説明したシフト還元構文解析についておさらいしましょう。

シフト還元構文解析では、スタックと入力文字列を使って、入力文字列を順番に読み込んでいきます。また、同時に生成規則を使って、スタックに積まれた記号列をより単純なものに置き換えていきます。具体的には、以下のような操作を繰り返します。

- スタックにプッシュ（シフト）
- スタックトップの記号列を規則の左辺で置き換える（還元） 

しかし、4.3で説明したシフト還元構文解析の説明では、どのような時にシフトを行い、どのような時に還元を行うかについては触れていませんでした。これではちゃんとしたアルゴリズムとは言えません。

シフト還元構文解析は特定の手法というより発想の概略とでもいうべきものであり、その具体的な実装の一つが**SLR(1)**になります。SLRはSimple LRの略で、LRは**Left-to-right Rightmost derivation**の略です。、左から右に読み込み、最右導出を行う方法とでも言えるでしょうか。最右導出についても第3章で説明しましたね。ちなみに端的に言うと、還元は導出のちょうど逆の操作になります。
