...
`D`が構文解析の際に最初に参照される記号（開始記号）です。このBNFを文脈自由文法による記述になおしていきましょう。
...
のような、`->`で区切られた右と左をあわせたものを**生成規則**と呼びます。$D$や$P$を**非終端記号**と呼び、$($は**終端記号**と呼びます。このようにして見ていくと、文脈自由文法とは、
...
何故、BNFという表記法でなく文脈自由文法の標準的な表記法に変換するかといえば、5章以降で示す種々の構文解析アルゴリズムの多くが、文脈自由文法をベースに構築されているからです。
...
JP = {
  ...
  "public class P8 { public static void main(String[] args) { System.out.println(3 + 5); }}", // クラス名をP8に修正
  ...
}
```
...
たとえば、Ruby言語を表す無限集合を`RB`と考えたとき、集合の和`RB ∪ DK`を考えることが出きます。`RB ∪ DK`は以下のようになります。
