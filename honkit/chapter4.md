...
まず、明らかにLL(1)では2つを区別することができません。なぜなら、最初のトークンが`user_type`だったとして果たしてこれが型名なのか変数名なのかを区別することは出来ないからです。もちろん、型名のテーブルを持っていれば可能ですがそれでは文脈自由言語の範囲を逸脱してしまいます。

LL(1)ではなくLL(2)アルゴリズムを使うとしてみます。すると、`user_type x`という2つのトークンが来た場合は明らかに型宣言であることがわかりますし、`foo ;`という2つのトークンが来た場合は式文であることがわかります。つまり、先読みトークン数を1から2に増やすことでより広い範囲の文法を認識できるようになったわけです。

さらに`k`は任意の整数で良いのですから、`k`を3に増やしても4に増やしても、あるいは5でも良いわけです。`k`の数を増やし過ぎると消費する記憶領域や実行性能面で問題が発生するのですが、その問題を脇においておけば`k`を増やすことでより広い範囲の文法を認識できるようになっていくのです。

## 4.4 LL(k)の限界

`LL(k)`では`k`の数を増やしていくことで表現能力を拡張できるわけですが、ここで1つの疑問が生まれます。`k`をいくらでも増やしていくことによって、任意の文脈自由言語を認識できるようになるのだろうか、というものです。この疑問に答えるためには複雑な議論が必要ですが、答えだけをいうとNOです。文脈自由言語であるが`LL(k)`では認識できない言語は必ず存在します。

たとえば、文脈自由言語の例として以下のようなものがあります。
...
`LL(1)`からはじまる下向き型構文解析アルゴリズムについて解説してきました。下向き型構文解析アルゴリズムは人間の直感あうことやエラーメッセージの出しやすさなどの利点があるのですが、素朴な`LL(1)`やそれを拡張した`LL(k)`でも表現力に限界があります。この点では上向き型の典型である`LR`やその派生系のアルゴリズム、とくに`LALR`に一日の長があります。

実際、少なくとも21世紀はじめ頃までは`LR`あるいはその変種が広く使われていました。しかし、2024年現在では`LL(k)`をより拡張した`LL(*)`あるいは`ALL(*)`を実装したANTLRも広く使われています。下向き型のアルゴリズムという意味では`LL(k)`や`ALL(*)`の「お仲間」と言えるPEGもさまざまな場面で使われるようになってきています（Python 3.10から正式な構文解析機はPEGベースです）。ともあれ、`LR` の変種が構文解析の世界で広く使われているのも間違いありません。

たとえば、この本をお読みの読者ならyacc（実際には互換製品であるbisonが主に使われていますが、以後yaccで統一します）という名前を一度は聞いたことがあるかもしれません。yaccはもっとも広く使われている構文解析器生成系の1つであり、LALR(1)アルゴリズムを採用しています。yaccは[Rubyの構文解析器](https://github.com/ruby/ruby/blob/db6b23c76cbc7888cd9a9912790c2068703afdd0/parse.y)を作るためなど、言語処理系の構文解析器を生成するために使われています。

とはいえいきなり`LR`や`LALR`を説明するのは難しいので、この節では`LR`をもっとも簡略化した`SLR`について説明します。

### SLRの概要

`SLR`を理解するために基本的な概念から説明していきましょう。`SLR`は以下の特徴を持っています：

- 単純なLR法（**S**imple LR）
- 入力を左から右に読み込む（**L**eft-to-Right）
- 最右導出の逆を行います（**R**ightmost derivation）

見ればわかる通り、SLRはSimple LRの略で、LRはLeft-to-right Rightmost derivationの略です。ここで、Left-to-rightは「左から右に文字列を読み込む」を指します。これは直感的にわかりやすいでしょう。Rightmost derivationは「最右導出を行う」という意味です。最右導出とは、文脈自由文法に関連した用語です。文脈自由文法から文を生成する過程には大きく分けて最左導出と最右導出があります。
...
といってもこれだけだとわけがわからないという方が大半ではないかと思います。しかし、実は読者の皆さんがこの章で既に見たシフト還元構文解析が理解できていれば、その入口に立ったようなものです。
...
しかし、素朴なシフト還元構文解析とSLRやLR法が大きく異なる点があります。それは構文解析表と呼ばれる表を用いてシフトするか還元するかを決定する点です。素朴なシフト還元構文解析ではスタックを毎回スキャンしていましたが、これではスタックのサイズが大きくなってきたときに遅くなるのが想像できますね。一方、SLRでは構文解析表を使ってシフトするか還元するかを決定するため、効率的に構文解析を行うことができます。
...
SLRでは、項目の集合を「状態」として扱います。初期状態から始めて、遷移を繰り返すことで全ての状態を構築していきます。
