...
とはいえ、いきなりLR(1)アルゴリズムを説明するのは難しいので、まずはそれをもっと簡略化したSLR(1)アルゴリズムについて説明します。その前に、4.3で概略を説明したシフト還元構文解析についておさらいしましょう。

シフト還元構文解析では、スタックと入力文字列を使って、入力文字列を順番に読み込んでいき、

- スタックにプッシュ（シフト）
- スタックトップの記号列が規則の右辺にマッチしたら、スタックトップの記号列を規則の左辺で置き換える（還元） 

どちらかの操作を繰り返すのでしたね。しかし、4.3で説明したシフト還元構文解析の説明では、どのような時にシフトを行い、どのような時に還元を行うかについては触れていませんでした。これではちゃんとしたアルゴリズムとは言えません。

実は、シフト還元構文解析は特定の手法というより発想の概略とでもいうべきものであり、その具体的な実装の一つがSLR(1)アルゴリズムになります。SLRはSimple LRの略で、LRは**Left-to-right Rightmost derivation**の略です。日本語で表現するなら、左から右に読み込み、最右導出を行う方法とでも言えるでしょうか。導出というのは還元の逆の操作です。
