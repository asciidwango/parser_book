...
用語として不正確なのを承知で言うなら、第2章で最初に実装したのは、バックトラックあり再帰下降構文解析器でPEGと呼ばれる手法の素朴な実装です。後で実装したのは、LL(1)**っぽい**再帰下降構文解析器と言えます。「再帰下降」という言葉は見慣れないもので疑問に思われる読者の方も多いと思います。ともあれ、その疑問は脇においておいて、第2章での実装が理解出来たなら、皆さんは直感的には構文解析アルゴリズムを理解していることになります。

この章では、2024年までに発表された主要な（いくつかは筆者の独断と偏見が入っています）構文解析アルゴリズムについて解説していきます。実用的には、紹介する構文解析アルゴリズムのほとんどについて、その構文解析アルゴリズムを使った構文解析器を生成してくれる**構文解析器生成系**が存在するからです。たとえば、おそらく最もメジャーな構文解析器生成系は、`LALR(1)`アルゴリズムを用いたyacc（正確には、GNUによる再実装であるbisonが現在は主流）でしょう。yaccに似た構文解析器生成系はC向けのyaccの他に、Ruby向けのracc、Java向けのJay、OCaml向けのocamlyaccなど数多くのバリエーションがあります。
...
小難しいことばかり言うのは趣味ではないので、早速、構文解析アルゴリズムの世界を覗いて見ましょう！　
...
　説明が抽象的過ぎるので具体例を挙げてみます。たとえば、下記の文法があったとします。ただし、入力の先頭と末尾を表すために `$` を使うものとします。
...
## 4.5 SLR(0) - 単純な上向き構文解析アルゴリズム

ここまでLL(1)からはじまる下向き型構文解析アルゴリズムについて解説してきました。下向き型構文解析アルゴリズムは人間の直感に合致してることやエラーメッセージの出しやすさなどの利点があるのですが、素朴なLL(1)やそれを拡張したLL(k)でも表現力に限界があります。この点では上向き型の典型であるLR parsingやその派生系のアルゴリズムに一日の長があります。

実際、20世紀末まではLR(1)アルゴリズムあるいはその変種が広く使われていました。しかし、現代ではLL(k)をより拡張したLL(*)やALL(*)を実装したANTLRも広く使われていますし、下向き型のアルゴリズムという意味では「お仲間」とも言えるPEGもさまざまな場面で使われるようになってきています。ともあれ、LR parsingの変種が構文解析の世界で広く使われているのも間違いありません。
...
いきなりLALRやLRを説明するのは難しいので、LRをもっとも簡略化した`SLR(0)`について説明します。

### SLR(0)法の概要

SLR(0)法は、LR構文解析の中でも、もっとも単純な形式です。この方法を理解するために、まずは基本的な概念から説明していきましょう。SLR(0)法は以下の特徴を持っています：

- 上向き構文解析（ボトムアップ解析）の一種です
- LR法の中でもっとも単純（**S**imple）な方式です
- 入力を左から右に読み込みます（**L**eft-to-Right）
- 最右導出の逆を行います（**R**ightmost derivation）
- 先読みを行いません（Look-ahead **0**）

### LR(0)項

SLR(0)法では、文法規則を`LR(0)項`（以下、単に項と書きます）という形に変換します。項は規則の右辺にドット（・）を挿入したものです。たとえば：
...
ある規則から得られる項集合は、その規則の左辺に対応する構文解析の途中結果の集合と見ることができます。規則の右辺は3つの要素からなるため、4つの項が得られます。

### 状態の構築

SLR(0)法では、項目の集合を「状態」として扱います。初期状態から始めて、遷移を繰り返すことで全ての状態を構築していきます。
...
この状態から、各記号（E, T, x）に対する遷移を考えます。例えば、xに対する遷移は
...
・の右側にある記号がxと一致している項を集めてくればいいわけですね。
...
すべての状態を構築したら、それをもとに構文解析表を作成します。表には以下のアクションが含まれます：

- シフト: 入力記号を読み込んでスタックにプッシュ
- 還元: スタック上の記号列を文法規則の左辺に置き換え
- 受理: 構文解析成功
- エラー: 構文エラー
...
先ほどの文法を使って、入力文字列 "x+x" を解析する過程を見てみましょう：
...
SLR(0)法は比較的単純ですが、扱える文法に制限があります。より複雑な文法を扱うためには、LALR(1)やLR(1)などの拡張された手法が必要となります。
