...
BNFでは、以下のような**規則**の集まりによって、文法を表現します。
...
本体の中に出てくる、他の規則を参照する部分（ここでは`term)を非終端記号と呼びます。非終端記号は同じBNF内で定義されている規則名と一致する必要があります。
...
また、その関数の引数は解析したい文字列との添字であるものとします。たとえば、`expression("1+2", 0)`は、文字列`"1+2*3"`の先頭から解析を開始することを示します。
...
規則の本体では`term`が最初に出現するので、規則`term`を適用します。つまり、`term("1+2*3")`となります。
...
今、文字列の0番目と規則を照合しようとしているわけですが、`"1+2*3"`の0番目の文字は`1`です。したがって、`NUMBER`の本体の内、`1`との照合に成功します。
...
### 2.3.2 内部ノード（Internal Nodes）

演算子や制御構造など、他のノードを子として持つノードです。

先程の例でいうと、`+`ノードと`*`ノードが内部ノードです。

### 2.3.3 葉ノード（Leaf Nodes）

数値や変数など、子を持たないノードです。例えば、`1`や`2`、`3`といった数値は葉ノードとなります。

### 2.3.4 ルートノード（Root Node）

抽象構文木の最上位に位置するノードです。抽象構文木の根となるノードであり、プログラム全体を表します。数式の例でいえば、全体を表す`+`ノードがルートノードとなります。

内部ノードとルートノードは排他でないことに注意してください。たとえば、`+`ノードは内部ノードであり、同時にルートノードでもあります。
...
```java
// 式を表すインタフェース
sealed interface Expression permits Addtion, Subtraction, Multiplication, Division, NumberT {}
// 加算を表すレコード
record Addtion(Expression left, Expression right) implements Expression {}
// 減算を表すレコード
...
インタフェース`Expression`は、抽象構文木のノードを表すインタフェースです。`Addtion`、`Subtraction`、`Multiplication`、`Division`、`NumberT`は、それぞれ加算、減算、乗算、除算、数値を表すレコードです。これによって、抽象構文木をJavaで表現することができます。

試しに`1 + 2 * 3`の抽象構文木をJavaで表現してみましょう。

```java
Expression ast = new Addtion(
  new NumberT(1),
  new Multiplication(
    new NumberT(2),
    new NumberT(3)
...
int eval(Expression e) {
  return switch(e){
    case NumberT n -> 
      n.value();
    case Addtion a -> 
      eval(a.left()) + eval(a.right());
    case Subtraction s -> 
      eval(s.left()) - eval(s.right());
    case Multiplication m -> 
      eval(m.left()) * eval(m.right());
    case Division d ->
      eval(d.left()) / eval(d.right());
  };
}
```

ノードの種類に応じてswitch式で処理を分岐しています。`NumberT`ノードの場合はその値を返し、`Addtion`ノードの場合は左右の子ノードを再帰的に評価して足し算を行います。同様に、`Subtraction`、`Multiplication`、`Division`ノードの場合も、それぞれの演算を行っています。
