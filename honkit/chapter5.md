...
| アルゴリズム        | 時間計算量     | 空間計算量 (解析表サイズ)         | 備考                                   |
| ------------------- | ------------- | ------------------------------- | -------------------------------------- |
| LL(1)               | O(n)          | O(|N| * |T|)                   |                                        |
| LL(k)               | O(n)          | O(|N| * |T|^k)                 | kが大きくなると表サイズが指数的に増大   |
| SLR(1)              | O(n)          | O(状態数_LR0 * (|T|+|N|))      | 状態数はLR(0)と同じ                    |
| LR(1)               | O(n)          | O(状態数_LR1 * (|T|+|N|))      | 状態数がSLR(1)より大幅に増加する可能性 |
| LALR(1)             | O(n)          | O(状態数_LR0 * (|T|+|N|))      | 状態数はLR(0)と同じ (LR(1)より小さい)   |
| PEG                 | O(2^n) (最悪) | O(n) (バックトラック用スタック) | Packrat Parsingで線形時間に改善可能    |
| Packrat Parsing     | O(n)          | O(n * |N|) または O(n * |P|)   | メモ化テーブルのサイズ                 |
