...
## 5.9 LR(0) - 単純な上向き構文解析アルゴリズム

LR（0）は、LR法の中でも最も単純な手法です。LRは**L**eft-to-right（左から右への入力）と**R**ightmost derivation（最右導出）の略です。 Left-to-rightは「左から右に文字列を読み込む」を指します。これは直感的にわかりやすいでしょう。Rightmost derivationは「最右導出を行う」という意味です。
...
LR(0)における構文解析表は、構文解析器が入力を解析する際に必要なアクション（シフト、還元、受理、エラー）を決定するための表です。この表は、各状態と各入力記号の組み合わせに対して、どのアクションを取るべきかを示します。
...
1. 与えられた文法の拡張：開始記号Sに対して、新しい開始記号S'と生成規則S' → Sを追加します。

2. 初期状態の作成：S' → ・S$という項目を含む閉包を計算し、これを初期状態（状態0）とします。

3. 状態の構築：初期状態から始めて、各状態にDOTO関数を適用し、新しい状態を生成します。すべての状態とその間の遷移が見つかるまでこの処理を続けます。

4. 構文解析表の作成：
- ACTION表：各状態iと終端記号aに対して、
  -  項目[A → α・aβ]が状態iにあり、GOTO(i, a) = jならば、ACTION[i, a] = "shift j"
  - 項目[A → α・]が状態iにあれば、A = S'なら、ACTION[i, $] = "accept"、それ以外なら、ACTION[i, a] = "reduce A → α"（すべての終端記号aについて）
- GOTO表：各状態iと非終端記号Aに対して、GOTO(i, A) = jならば、GOTO[i, A] = j

5. 競合の解決：同じセルに複数のアクションが入る場合、LR(0)では競合が発生します。これはLR(0)できない文法の証拠です。
...
このように、LR(0)では構文解析表を参照しながら、入力文字列を左から右に読み込み、適切なタイミングでシフトと還元を繰り返すことで構文解析を行います。LR(0)は比較的単純ですが、扱える文法に制限があります。次にLR(0)を拡張して先読みを考慮したSLR(1)法について説明します。

## 5.10 SLR(1) - 先読みを考慮した上向き構文解析アルゴリズム

`SLR(1)`（Simple LR(1)）法は、`LR(0)`を先読み情報で強化した上向き構文解析アルゴリズムです。`SLR(1)`では、LR(0)項目集合を用いて状態を構築し、さらに文法のFOLLOW集合を利用して解析表を作成します。これにより、LR(0)法で生じるシフト・還元（shift-reduce）コンフリクトや還元・還元（reduce-reduce）コンフリクトを解消することが可能になります。
