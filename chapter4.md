...
LL(k)ではkの数を増やしていくことで表現能力を拡張できることがわかったわけですが、ここで一つの疑問が出てきます。LL(k)のkをいくらでも増やしていくことによって、任意の文脈自由言語を認識できるように拡張可能なのだろうか、というものです。実際のところ、文脈自由言語ではあるがLL(k)では認識できない言語は存在します。ただ、そのような証明はやや難解ですし、一見LL(k)で認識できないように見える言語もleft factoringによって認識可能になることもあります。

実際、現在存在しているプログラミング言語の文法をLL(k)で十分表現出来るかどうかははっきりわかっていません。実用的なプログラミング言語の文法を表現するにはLL(k)だけだと色々しんどいという共通認識のようなものはありますが、できないという証明があるわけでもありません。

とはいえ、LL(k)**言語**でなくLL(k)**文法**では直観的に表現しづらい対象（※言語と文法の区別を後で記述する必要がありそう）は存在します。たとえば、以下のBNFは`var x;`や`var x: Int;`のような変数宣言を表しています。ここで、`|`の左辺は型がない場合の宣言で、右辺は型がある場合の宣言です。


```
var_declaration = "var" identifier ";" | "var" identifier ":" type ";"
identifier = [a-zA-Z_] [a-zA-Z0-9_]
```

共通のプレフィクスである`"var"`はLL(k)でk = 3にすることで回避出来ますが、その後の`identifier`が問題です。`identifier`は定義をみればわかるように任意の長さをとるかもしれないので、固定長の先読みしかできないLL(k)では取り扱えないのです。


## 4.5 LR(1) - 代表的なボトムアップ構文解析アルゴリズム

　さて、ここまででLL(1)からはじまるトップダウン型構文解析アルゴリズムについて解説してきました。トップダウン型構文解析アルゴリズムは人間の直感に合致してることやエラーメッセージの出しやすさなどの利点があるのですが、悲しいことに素朴なLL(k)アルゴリズムでは表現力に限界があります。そのようなこともあって、構文解析の世界ではより表現力の高いLR(1)アルゴリズムあるいはその変種が広く使われています。

　この本をお読みの読者なら聞いたことがあるかもしれないyacc(その互換製品であるbison。以後yaccで統一します)はLALR(1)アルゴリズムを採用した構文解析器生成器です。yaccはRubyの構文解析器をはじめ、色々な場所で使われています。
...
## 4.8 - 解析表現文法（Parsing Expression Grammar） - 構文解析アルゴリズムのニューカマー

TBD

## 4.9 - Generalized Parsing

TBD

## 4.10 - LL(*) と ALL(*) - 実用性に裏打ちされた強力な構文解析アルゴリズム

4.4でLL(k)では直観的に表現しづらい言語があることを説明しました。実際、そのような制限があるため、過去のある時期まではLR法をベースとしたアルゴリズムが主流であったという側面があったのですが、一方でLL(k)を拡張することで欠点を補おうとするアプローチも連綿として研究されてきました。

[ANTLR](https://www.antlr.org/)という世界中で使われている構文解析器生成系の作者である[Terence Parr](https://parrt.cs.usfca.edu/)氏もその一人です。彼はPCCTS(Purdue Compiler Construction Tool Set)の後継としてANTLRの開発を始めたのですが、よりツールを実用的にしていく過程でLL(*)やALL(*)アルゴリズムを見出したようです。筆者が記憶している限りでは、LL(*)アルゴリズムの概要自体は2000年代後半にTerenceさんがinformalな形で公開しており、ANTLR V3に実装されていたのですが、論文として発表されたのはPLDI 2011(Programming Language Design and Implementation 2011)という国際会議が初出でした。

このLL(*)アルゴリズムは4.4にあった以下の例をそのまま取り扱うことが出来るという強力な利点を持っています。
...
一言で言うと、LL(*)はLL(k)のkが固定の整数であるという制限を取っ払って先読みを無限長にするというアプローチをとりました。

<!-- LL(*)の詳細をどこまで説明するべきか -->

このアルゴリズムの採用によって、ANTLR V3は柔軟性の高い文法を簡潔に表現できるようになりました。

さらに、Terence氏はANTLR V4でLL(`*`)の発展系であるALL(`*`)アルゴリズムを発表しました。ALL(`*`)アルゴリズムは文脈自由言語全てを取り扱えるアルゴリズムの一つでもあります。古典的なLL(k)アルゴリズムにあった、必ず線形時間で解析を終えられるという良い性質はなくなりましたが、実用的にはほとんどのケースで線形時間で解析を終えられるとTerence氏は主張しています。

ANTLR V4は色々なところで使われているツールであり、使い勝手の良さは実証済みでもあるため、ALL(`*`)アルゴリズムが実用的なケースではうまく動作するという主張には説得力があります。構文解析アルゴリズムは実用で使われてなんぼという面はありますから、有用性の主張には実験が不可欠になります。豊富な利用実績というのは有用性を主張するときにこの上ない材料になります。

## 4.11 - 構文解析アルゴリズムの計算量と表現力の限界 
