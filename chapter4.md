...
　この本をお読みの読者ならきっと聞いたことがあるyacc(bison)はLALR(1)アルゴリズムを採用した構文解析器生成器です。yaccやその互換品であるbisonはRubyの構文解析器をはじめ、色々な場所で使われています。
