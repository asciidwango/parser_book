...
- **JavaCC**：
  ...
  - Java向け
- **ANTLR**：
  - ALL(*)[^allstar]を採用（拡張されたLL系を採用）
  - 多言語対応
- **各種PEGパーサジェネレータ**：PEGを採用

これらのツールについては第6章で詳しく説明しますが、本章でアルゴリズムを理解することで、各ツールがなぜそのアルゴリズムを選んだのかが見えてくるはずです。

[^allstar]: ANTLR 4が採用する解析戦略。必要なだけ先読みを伸ばすLL(*)を動的に評価（Adaptive LL）する方式で、固定長先読みのLL(k)を実用的に一般化したものです。
...
ここで、どちらを選ぶかを事前に決める必要があります。先読み文字が`(`なので、`P -> ( P ) P`を選択します。この規則をスタックに追加します：
...
6. スタックの末尾`X`が規則`P -> X`の右辺と一致。1つの記号を`P`に**還元**（この実装はシフトする前に、可能な還元をすべて試みます）：
...
8. スタックの末尾`(, P, )`が規則`X -> ( P )`の右辺と一致。3つの記号を`X`に**還元**：
...
9. スタックの末尾`X`が規則`P -> X`の右辺と一致。1つの記号を`P`に**還元**：
...
11. スタックの末尾`$, P, $`が規則`D -> $ P $`の右辺と一致。3つの記号を`D`に**還元**：
...
`E`は複数のトークンで始まり得ますが、それ自体は問題ではありません。困るのは、**同じトークンで始まる規則が複数ある場合に、最初の1トークンだけではどの規則を選ぶか決められない**ことです。
...
のように、`id`が式の先頭として再利用される文法では、先読み1トークンだけではどちらの規則を適用するのか判断できません。トークンが複数の構文で使い回されると、このような曖昧さが生じます。ここで触れた「1トークンで判断できない」状況は、後で扱う共通前置辞問題（5.9.7）の最も短い形だと考えてください。
...
ここで`X₁, X₂, ...`は、規則の右辺に並ぶそれぞれの記号（位置付きで区別したもの）を表します。
...
ここで`X1, X2, ... Xn`は、規則の右辺に並ぶそれぞれの記号を位置付きで区別するための書き方です（同じ非終端記号でも、出現位置ごとに別の`Xi`として扱います）。
...
先ほど（「例で学ぶシフト還元構文解析」（5.6.3節）で示した）`DyckShiftReduce`の実装を思い出してください：
...
還元を優先する`DyckShiftReduce`では、次のように進みます：

1. スタック末尾の`X`を`P`に還元する
2. `)`をシフトしてスタックに積む
3. その後、`( P )`を`X`に還元する
...
前節ではシフト還元構文解析のために`$`や`X`を導入した拡張文法を使いましたが、ここでは状態の見通しを優先して元の簡潔な文法に戻しています（拡張文法でも同様にオートマトンを作れますが、状態数が増えて図が煩雑になるためです）。
...
状態名が`I0`のような番号になっているのは、後で導入するLR(0)項集合に対応づけるためです。イメージしやすいように、ここでは次のように読み替えてください：

- I0: まだ何も読んでいない初期状態
- I1: `D`を読み終えて終端`$`を待っている状態
- I2: 入力全体を受理した状態
- I3: 括弧列`P`を読み進めている状態
- I4: `(`を読んだ直後で、その中身の`P`をこれから処理する状態
- I5: `( P`まで読み、対応する`)`を待っている状態
