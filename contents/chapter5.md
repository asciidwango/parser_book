...
3. `X₁`が非終端記号なら：
   ...
   - `X₁`が**nullable**なら`X₂`も確認（以下同様）
4. すべての`Xᵢ`が**nullable**なら、`ε`を`FIRST(A)`に追加
...
このアルゴリズムにより、「この状態で何が期待されるか」をすべて列挙できます。
...
% 遷移を描画
\draw[->, thick] (I0) edge[bend left=10] node[above] {E} (I1);
\draw[->, thick] (I0) edge[bend right=10] node[below] {T} (I2);
\draw[->, thick] (I0) edge[bend right=20] node[below left] {id} (I3);
\draw[->, thick] (I1) edge[bend left=10] node[above] {\$} (I4);
\draw[->, thick] (I1) edge node[right] {+} (I5);
\draw[->, thick] (I5) edge node[above] {T} (I6);
\draw[->, thick] (I5) edge[bend left=15] node[below] {id} (I3);
...
先程の文法に対して、LR(0)オートマトンを構築する過程を見ていきましょう：
...
構築した構文解析表を使って、入力 `id + id $` を解析してみましょう。各ステップでは現在の状態と入力記号からアクションを決定します：

**ステップ1**：最初のidを読み取り

- スタック: [0] （初期状態）
- 入力: id + id $
- アクション: ACTION[0, id] = s3 （状態0でidを読んだらシフトして状態3へ）
- 実行: idをシフトして状態3へ遷移
...
- スタック: [0, 3] （状態0の上に状態3）
- 入力: + id $
- アクション: ACTION[3, +] = r3 （T → idで還元）
- 実行: 
  1. T → idの右辺は1個の記号なので、スタックから1個削除して [0] に
  2. 非終端記号Tをスタックトップの状態0で処理：GOTO[0, T] = 2
  3. 状態2をスタックにプッシュして [0, 2] に

**ステップ3**：TをEに還元

- スタック: [0, 2]
- 入力: + id $
- アクション: ACTION[2, +] = r1 （E → Tで還元）
- 実行:
  1. E → Tの右辺は1個の記号なので、スタックから1個削除して [0] に  
  2. 非終端記号Eを処理：GOTO[0, E] = 1
  3. 状態1をプッシュして [0, 1] に
...
- スタック: [0, 1, 5]
- 入力: id $
- アクション: ACTION[5, id] = s3 （idをシフトして状態3へ）
- 実行: idをシフトして状態3へ遷移
...
- アクション: ACTION[3, $] = r3 （T → idで還元）
- 実行:
  1. T → idの右辺は1個の記号なので、スタックから1個削除して [0, 1, 5] に
  2. 非終端記号Tをスタックトップの状態5で処理：GOTO[5, T] = 6
  3. 状態6をプッシュして [0, 1, 5, 6] に

**ステップ7**：E + TをEに還元（式全体を結合）

- スタック: [0, 1, 5, 6]
- 入力: $
- アクション: ACTION[6, $] = r2 （E → E + Tで還元）
- 実行:
  1. E → E + Tの右辺は3個の記号なので、スタックから3個削除して [0] に
  2. 非終端記号Eをスタックトップの状態0で処理：GOTO[0, E] = 1
  3. 状態1をプッシュして [0, 1] に
...
- スタック: [0, 1]
- 入力: $
- アクション: ACTION[1, $] = acc （受理）
- 実行: 解析成功！入力文字列は文法に適合している
