...
スタックを使った下向き構文解析の動作を理解したところで、これをJavaで実装してみましょう。多くの場合、スタックを明示的に使わずに**再帰呼び出し**を使って実装できます。既に第3章で実装した`SimpleJsonParser`は、予測型再帰下降構文解析の一種で、実装もほぼ同じです。
...
1. **FIRST集合に基づく登録**：
   - `FIRST(α)`内の各終端記号`a`に対して、表の`[A, a]`に規則`A → α`を登録
   - つまり、`α`が`a`で始まる可能性がある場合、`a`を見たらこの規則を適用

2. **空文字列規則の処理**：
   - `ε ∈ FIRST(α)`の場合（`α`が空文字列を導出可能）
   - `FOLLOW(A)`内の各記号`b`に対して、表の`[A, b]`に規則`A → α`を登録
   - つまり、`A`の後に`b`が来る可能性があり、`A`が空になれる場合

3. **コンフリクトのチェック**：
   - 同じセル`[A, a]`に複数の規則が登録される場合、その文法はLL(1)ではない
   - これを**LL(1)コンフリクト**と呼ぶ

具体例で見てみましょう。以下の文法を使います：
...
先ほど計算したFIRST集合とFOLLOW集合を使って、各規則をLL(1)表に登録していきます：

1. 規則 E → T E' の処理：

- `FIRST(T E') = FIRST(T) = {(, id}`
- 表の`[E, (]`と`[E, id]`に「E→TE'」を登録
